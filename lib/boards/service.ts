...
// The public boards a given person has published, for their profile page. This
// is the public, read-only view of someone else's work: private boards never
// appear here, not even to the owner viewing their own profile — a profile is a
// public page, so it shows exactly what a stranger would see. Ranked like the
// shared grid (stars, then recency) so a person's best-known board leads.
export async function listPublicBoardsByOwner(ownerId: string): Promise<BoardSummary[]> {
  const viewer = await getCurrentUser()

  const rows = await db
    .select()
    .from(boards)
    .where(and(eq(boards.ownerId, ownerId), eq(boards.isPublic, true)))

  const summaries = await starSummaries(rows, viewer?.id ?? null)
  return summaries.sort(
    (a, b) =>
      b.starCount - a.starCount || new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime(),
  )
